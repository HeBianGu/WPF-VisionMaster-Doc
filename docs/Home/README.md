...
本文档已合并 `Source/Apps/H.App.VisionMaster/README.md` 中的主应用说明，并按“快速上手 → 主应用能力 → 架构模块 → 控件机制 → 二次开发”的顺序整理。
...
- [环境要求](#环境要求)
- [快速开始](#快速开始)
- [主应用 `H.App.VisionMaster` 概览](#主应用-happvisionmasteropencv4-概览)
- [常用模块说明](#常用模块说明)
...
Source/
├── Apps/
│   └── H.App.VisionMaster/       # 主 WPF 应用
├── VisionMaster/                         # 机器视觉核心业务模块
│   ├── H.Components.VisionDiagrams.OpenCV/            # OpenCV 基础能力
│   ├── H.VisionMaster.Camera/            # 相机接入
│   ├── H.Components.VisionDiagram/          # 视觉节点数据
...
├── H.App.VisionMaster.csproj    # 主应用项目文件，定义目标框架、项目引用和资源配置
├── VisionIdentifyDataContext.cs         # 身份认证数据库上下文，用于用户、角色、权限等数据
├── Commands/                            # 主应用命令扩展，例如窗口、缩放、URI、流程图相关命令
├── DiagramDatas/                        # 主应用流程图数据类型，例如 VisionDiagramData
...
可通过 Visual Studio 将 `H.App.VisionMaster` 设置为启动项目后运行，或使用命令行：

```powershell
dotnet run --project Source/Apps/H.App.VisionMaster/H.App.VisionMaster.csproj
```

也可以直接在 Visual Studio 中将 `H.App.VisionMaster` 设置为启动项目后按 `F5` 运行。

## 主应用 `H.App.VisionMaster` 概览

`H.App.VisionMaster` 是主 WPF 应用，面向机器视觉流程搭建和演示。应用默认创建 `机器视觉流程`，流程画布尺寸为 `2000 x 1500`，用户可以在流程资源面板中拖拽节点，组合成完整视觉处理链路。
...
主应用通过 `H.Components.VisionDiagrams.OpenCV*` 相关模块提供 OpenCV 视觉节点能力，并引入：
...
- 新增 ONNX 节点时，应在 `VisionDiagramData.CreateLocalNodeGroups()` 中注册节点或节点分组。
...
主应用入口位于 `Source/Apps/H.App.VisionMaster/App.xaml.cs`。`App` 继承自 `ApplicationBase`，应用启动时会完成服务注册、配置加载、启动页、登录、主窗口创建和退出清理等工作。
...
protected override void Configure(IApplicationBuilder app)
{
    ...
    app.UseSplashScreenOptions(x =>
    {
        ...
        x.Background = "pack://application:,,,/H.App.VisionMaster;component/Assets/splash.jpg";
    });
}
```
...
```text
我的文档/HeBianGu/H.App.VisionMaster
```
...
```text
D:\VisionData\MyCompany\H.App.VisionMaster\Default\4.0
```

登录用户目录会变为：

```text
D:\VisionData\MyCompany\H.App.VisionMaster\<UserName>\4.0
```
...
`Project` 表示一个完整的机器视觉工程，是用户保存、打开和运行的业务单位。主应用中的项目对象基于 `VisionProjectItemBase` 扩展，默认实现位于 `Source/Apps/H.App.VisionMaster/Projects`。
...
默认项目通过 `VisionProjectItem.CreateDiagramData()` 创建 `VisionDiagramData`，流程画布默认尺寸为 `2000 x 1500`。项目序列化默认使用 `NewtonsoftJsonSerializerService`，项目文件保存目录来自 `AppPaths.Instance.Project`。
...
| `Source/Apps/H.App.VisionMaster/Projects` | 主应用项目实现 |
...
- `ObservableCollection<IVisionDiagramData>` 中的实际类型可能是 `VisionDiagramData`。
...
主应用中的默认流程图类型是 `VisionDiagramData`，继承自 `VisionDiagramDataBase`，并实现 `IVisionDiagramData` 相关能力。
...
机器视觉应用中除了流程编排，还需要在图像上完成 ROI 选择、点线圆标注、测量图形绘制、像素拾取、卡尺区域编辑等交互。相关能力主要位于 `Source/VisionMaster/H.Controls.ShapeBox`。
...
<ContentPresenter Content="{Binding SelectedDiagramData}">
    <ContentPresenter.Resources>
        <DataTemplate DataType="{x:Type ld:VisionDiagramData}">
            <!-- VisionDiagramData 的专用呈现界面 -->
        </DataTemplate>
    </ContentPresenter.Resources>
</ContentPresenter>
```

当 `SelectedDiagramData` 是 `VisionDiagramData` 时，界面自动使用该模板显示视觉流程图、图像预览、历史结果、节点结果等内容。
...
| 场景 | 数据对象 | 呈现方式 |
| --- | --- | --- |
| 主流程图区域 | `VisionDiagramData` | `ContentPresenter` + `DataTemplate DataType` |
...
<Form
    ...
    UseTabNames="{x:Static VisionTabNames.ResultParameters}" />
```
...
[DefaultValue(128.0)]
[Range(0, 255)]
[Display(Name = "阈值", GroupName = VisionTabNames.RunParameters, Description = "二值化阈值", Order = 10)]
...
{
    UseGroup = true,
    UseGroupNames = VisionTabNames.RunParameters
};
```
...
{
    x.UseGroupNames = VisionTabNames.RunParameters;
    x.UseCommand = false;
});
```
...
{
    x.UseGroupNames = "基础信息,数据," + VisionTabNames.DisplayParameters;
});
```
...
- 运行参数放到 `VisionTabNames.RunParameters`。
- 结果参数放到 `VisionTabNames.ResultParameters`，必要时加 `ReadOnly(true)`。
...
```csharp
[Display(Name = "核大小", GroupName = VisionTabNames.RunParameters, Description = "滤波核大小", Order = 10)]
public Size KSize
{
    get { return _ksize; }
    set
...
例如新增一个“自定义阈值”节点，可在 `Source/VisionMaster/H.Components.VisionDiagrams.OpenCV/NodeDatas` 下新增文件：
...
public class CustomThresholdNodeData : OpenCVNodeDataBase, IBlurGroupableNodeData
{
    ...
    [DefaultValue(128.0)]
    [Display(Name = "阈值", GroupName = VisionTabNames.RunParameters, Description = "像素值大于该阈值时输出最大值")]
    public double Threshold
    {
        get { return _threshold; }
        set
    ...
    [DefaultValue(255.0)]
    [Display(Name = "最大值", GroupName = VisionTabNames.RunParameters, Description = "二值化后的最大像素值")]
...
- 运行参数使用 `Display(GroupName = VisionTabNames.RunParameters)`。
- 结果参数使用 `Display(GroupName = VisionTabNames.ResultParameters)`，如需被条件节点引用，可参考已有节点使用 `Expressionable`。
...
- 主应用相关说明统一维护在根目录 `README.md`，避免 `Source/Apps/H.App.VisionMaster/README.md` 与根文档重复。
